...
struct DataThread
{
    ...
    int max_latency;
    double latency;
    unsigned int latencies[MAX_SIZE];
};
...
do_work(void * data_th) {
    ...
    do {
        ...
        p_dth_th->latency = diff-SLEEP;
        if (p_dth_th->latency < 0) {
            DEBUG_PRINTF("LATENCIA NEGATIVE: %f TIME AFTER SLEEP: %ld TIME BEFORE SLEEP: %ld\n",
                            p_dth_th->latency, end.tv_nsec, intermediate.tv_nsec);
            continue;
        }

        if (p_dth_th->max_latency < p_dth_th->latency) {
            p_dth_th->max_latency = p_dth_th->latency;
        }
            
        p_dth_th->latency_avg = p_dth_th->latency + p_dth_th->latency_avg;
        p_dth_th->latencies[p_dth_th->it] = (unsigned int)p_dth_th->latency;
        
        p_dth_th->it++;
        
    } while (intermediate.tv_sec-begin.tv_sec <= SECS);
    
    p_dth_th->latency_avg = (p_dth_th->latency_avg/p_dth_th->it);
